...
When running Bazel builds on [[../nix/nix]], builds using the default `rules_java` configuration will fail when packaging JARs or compiling source JARs because the pre-built `singlejar_local` or `ijar` C++ binaries are dynamically linked against standard FHS glibc paths (such as `/lib64/ld-linux-x86-64.so.2`).
