...
- [[cognee]]: Open-source topological memory engine combining knowledge graphs, vectors, and relational storage.
- [[mem0]]: Open-source personalized memory layer with multi-level context extraction and vector search.
- [[letta]]: Stateful agent framework and LLM operating system featuring hierarchical self-editing memory.
- [[graphiti]]: Open-source temporal knowledge graph engine designed for dynamic agent memory and entity tracking.
- [[cmem]]: Persistent memory stream and context compressor tailored for coding agents like Claude Code and OpenClaw.
- [[memory-store]]: Cognitive memory platform and company brain by Julep AI with Model Context Protocol (MCP) integration.
- [[langmem]]: Long-term memory SDK by LangChain supporting semantic, episodic, and procedural memory extraction.
- [[honcho]]: User modeling and natural language plasticity memory engine by Plastic Labs.
- [[supermemory]]: Open-source second brain and context engine connecting external data to LLMs via MCP.
- [[hipporag]]: Neurobiologically inspired multi-hop memory framework using Personalized PageRank on knowledge graphs.
- [[hermes-memory-providers]]: Survey of external memory provider plugins in Hermes Agent (OpenViking, Holographic HRR, Hindsight, RetainDB, ByteRover).
